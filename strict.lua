...
-- highway=elevator
-- bug des guidages sur batiment/landuse
-- wheelchair bad
-- surface
--
-- Begin of globals

bollards_whitelist = { [""] = true, ["border_control"] = true, ["toll_booth"] = true, ["sally_port"] = true, ["gate"] = true, ["turnstile"] = true, ["entrance"] = true}
...
function node_function (node)
  ...
  local wheelchair = node.tags:Find("wheelchair")

  if wheelchair ~=nil and ("no" == wheelchair) then
    node.bollard = true;
  end
...
function way_function (way)
  ...
  -- First, get the properties of each way that we come across
    ...
    local incline = way.tags:Find("incline")
    local wheelchair = way.tags:Find("wheelchair")

  -- Second parse the way according to these properties

  if wheelchair == nil or ("yes" ~= wheelchair) then
    return 0
  end
  ...
  if highway ~=nil and ("steps" == highway) then
    return 0
  end
